...
def depth_limited_search(problem, limit=50):
    ...
    node = Node(problem.initial)
    return re_dls(node,problem,limit)

#tai 1 state cua node se luon co it nhat la 2 action co the dua ra
# nen node.expand luon co child vi vay ko bao gio ra ket qua la failure
# chi co 2 kq la cut-off hoac solution

#ngoai ra problem nay co infinite non-goal path nen neu ko dat limit thi
# code se roi vao inifinite loop
def re_dls(node,problem,limit):
    if problem.goal_test(node.state):
        ...
        for child in node.expand(problem):
            re = re_dls(child,problem,limit-1)
            if re != "cut-off":
                return re
        return "cut-off"#ko the nao la None


''' IMPLEMENT THE FOLLOWING FUNCTION '''
def iterative_deepening_search(problem):
    """See [Figure 3.18] for the algorithm"""
    for lim in range(1,INFINITE):
        res = depth_limited_search(problem,lim)
        if res != "cut-off":
            return res
...
#     result2 = depth_limited_search(problem,8)
#     if result2 == "cut-off":
#         print("Change your limit")
#     else:
#         print(result2.solution())
    

#     result3 = iterative_deepening_search(problem)
